...
class DateManager {
    companion object {
        ...
        fun getCurrentDate(): Date {
            val timeZone = GregorianCalendar(TimeZone.getTimeZone("GMT+8"))
        ...
        fun getCurrentDateOriginalTimeZone(): Date {
            val timeZone = GregorianCalendar(TimeZone.getTimeZone("GMT"))
            val year = timeZone.get(Calendar.YEAR)
            val month = timeZone.get(Calendar.MONTH) + 1
            val day = timeZone.get(Calendar.DAY_OF_MONTH)
            val sdf = java.text.SimpleDateFormat("yyyy-MM-dd")
            ...
            Log.d("ORIGINAL TIMEZONE: ", sdf.parse("$year-$month-$day").toString())
            return sdf.parse("$year-$month-$day")
        }

        @SuppressLint("SimpleDateFormat")
        fun getCurrentTimeOriginalTimeZone(): Date{
            val timeZone = GregorianCalendar(TimeZone.getTimeZone("GMT"))
            ...
            Log.d("ORIGINAL TIME: ", sdf.parse("$hour:$minute $am_pm").toString())
